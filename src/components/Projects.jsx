...
const Projects = () => {
    ...
    return (
        <motion.section
            ...
            id="projects"
            className="w-11/12 mx-auto py-20 text-white border-b border-sky-900/40 flex justify-center items-center min-h-screen"
        >
                ...
                <div className={`w-full p-6 grid grid-cols-1 md:grid-cols-2 ${projects.length === 1 ? "lg:grid-cols-1" : "lg:grid-cols-3 "} gap-5 place-items-center`}>
                    {projects.map((project) => (
                        <motion.div
                            key={project.name}
                            whileHover={{ y: -5 }}
                            className="rounded-lg border min-h-full border-sky-900/40 bg-sky-950/20 hover:bg-slate-900/50 transition-all duration-300 p-4 group cursor-pointer"
                        >
